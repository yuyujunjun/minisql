...
vector<int> query_on_index_from_file(Table table, Attribute index,int QAQ,int QWQ, string __indexname__) {
	
	Bplus<int>* bp;
		//int key = 0;
		bp=read_from_block(table, index, QAQ, __indexname__);
		bool min, max;
		min = max = false;
		if (minQAQ_i == QAQ)min = true;
		if (maxQwQ_i == QWQ)max = true;
		return bp->query_on_btree(QAQ,QWQ,min,max);
	//return 0;
}
vector<int> query_on_index_from_file(Table table, Attribute index, string QAQ, string QWQ,string __indexname__) {
	bool min, max;
	min = max = false;
		Bplus<string>*bp;
		string key = "";
		...
		if (QAQ == minQAQ_s)min = true;
		if (QWQ == maxQwQ_s)max = true;
		return bp->query_on_btree(QAQ,QWQ,min,max);
		/*	int front, index;
		front = index = 0;
		Btree_node<string>* queue[100];
...
vector<int> query_on_index_from_file(Table table, Attribute index, float QAQ,float QWQ, string __indexname__) {
		Bplus<float>*bp;
		float key = 1.1;
		bp = read_from_block(table, index, key, __indexname__);
		bool min, max;
		min = max = false;
		if (minQAQ_f == QAQ)min = true;
		if (maxQwQ_f == QWQ)max = true;
		return bp->query_on_btree(QAQ,QWQ,min,max);
//	return 0;
}

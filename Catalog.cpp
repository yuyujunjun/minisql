...
string GetAttributeOfIndexByName(string TableName, string AttributeName)
{
	ifstream in;
	Attribute myattr;
	char ch;
	...
	while (!in.eof())
	{
		...
		//过滤空格，转行符
		if (s1 != "" && s2 != "" && s3 != "")
			//表明相同则打开对应的所有索引catalog文件，直到属性相同为止
			if (TableName == s3)
			{
				myattr = GetAttributeOfIndex(s1, s3);
				if (myattr.attr_name == AttributeName)
				{
					in.close();
					in.clear();
					return s1;
